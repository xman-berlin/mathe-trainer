...
- **Guided Practice Plan**: From the home page, „Übung starten“ runs Mathe → Deutsch → Wochentage/Monate/Alphabet → Uhrzeit using daily goals (sequence blocks fixed at 5); type selectors locked during Mathe/Uhrzeit; pause/resume via Zurück and „Weiterüben“
...
- **Englisch / Übersetzung**: EN see+hear (`en-GB` TTS) → type German meaning; admin-configurable thematic lists (pairs with optional context line); own home tile, daily goal, and weight-based sessions
- **Home**: Four practice tiles (Mathe, Uhrzeit, Deutsch, Englisch); compact streak bar with coins links to Erfolge (no separate Erfolge tile; streak removed from hero profile)
...
- **Adaptive Difficulty**: Per-type difficulty levels (🐭 Maus → 🐉 Drache, 6 tiers) that automatically adjust based on performance — level up after 5 correct in a row, level down after 3 of last 5 wrong; persisted to Supabase. Higher tiers mix in lower-level review every 5th task (80/20). Addition/subtraction Löwe uses hundreds only (100–1000); Drache uses tens+hundreds with ones digit 0 (100–1000); Löwe/Drache are not blocked by a stale Zahlenraum ≤ 100 setting.
